<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00C7B7,100:1a1b27&height=200&section=header&text=Karan%20Thengane&fontColor=ffffff&fontSize=44&fontAlignY=35&desc=Software%20Engineer%20%C2%B7%20Full-Stack%20Developer&descAlignY=55&descSize=18" width="100%" alt="header"/>

<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=26&duration=3000&pause=800&color=00C7B7&center=true&vCenter=true&width=680&lines=Elixir%2FPhoenix+%C2%B7+MERN+%C2%B7+Next.js+%C2%B7+Java;Open+Source+Contributor+%40+CNCF;46+merged+PRs+in+production;ICPC+Regionalist+%7C+850%2B+LeetCode" alt="Typing SVG" />

<p>
  <a href="https://karanthengane.vercel.app/"><img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white"/></a>
  <a href="https://www.linkedin.com/in/karanthengane/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="mailto:karanthengane23@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
  <a href="https://leetcode.com/u/karanthengane404/"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/></a>
</p>

</div>

---

### 👨‍💻 About Me

- 🎓 **Computer Science graduate** and software engineer who loves shipping features that reach real users
- 💼 **46 merged PRs** into a production ESG platform at **Credibl** (Feb–Jun 2026) — Elixir/Phoenix, PostgreSQL, SAP ETL pipelines. That work lives on my work account: **[@karanCredibl](https://github.com/karanCredibl)**
- 🌱 Contributing to CNCF projects — **[Apicurio Registry](https://github.com/Apicurio/apicurio-registry)** (Red Hat / IBM) and **[KubeEdge Ianvs](https://github.com/kubeedge/ianvs)**
- 🏆 **ICPC Asia West Regionals — Honorable Mention** · **850+** problems solved on LeetCode
- 💬 Ask me about **backend, full-stack, databases**, or **DSA**
- 📫 Reach me at **karanthengane23@gmail.com**

---

### 🌐 Open Source

| Project | Contribution |
|---------|--------------|
| **[Apicurio Registry](https://github.com/Apicurio/apicurio-registry)** <br/> *CNCF Sandbox · Red Hat / IBM* | Found and reported a `priority/high` bug where the A2A agent and MCP tool discovery endpoints did exact matching despite documenting partial match — traced through the REST and SQL search layers, then opened the Java/Quarkus fix with JUnit + RestAssured tests. Also landed a docs fix and reported a UI filter-state bug. |
| **[KubeEdge Ianvs](https://github.com/kubeedge/ianvs)** <br/> *CNCF* | Restored reproducible benchmark runs by fixing broken configuration paths in the Cityscapes semantic-segmentation benchmark and removing hardcoded developer paths from the smart-coding example. |

---

### 🛠️ Tech Stack

<div align="center">

**Languages**

<img src="https://skillicons.dev/icons?i=elixir,java,python,cpp,ts,js&theme=dark" alt="languages"/>

**Backend & Frontend**

<img src="https://skillicons.dev/icons?i=phoenix,nodejs,express,react,nextjs,tailwind&theme=dark" alt="frameworks"/>

**Databases & Tools**

<img src="https://skillicons.dev/icons?i=postgres,mongodb,mysql,docker,git,linux&theme=dark" alt="tools"/>

</div>

---

### 🚀 Featured Projects

| Project | Description | Tech |
|---------|-------------|------|
| **[PDF-RAG — Chat with PDF](https://github.com/karanthengane23/pdf-rag)** | Full-stack AI app for real-time, context-aware querying of PDF documents, with async embedding generation | Next.js · BullMQ · Qdrant · HuggingFace |
| **[Samvad — Real-Time Chat](https://github.com/karanthengane23/Samvad1)** ([Live](https://samvad1-frontend.onrender.com)) | Instant messaging app with an AI assistant | MERN · Socket.IO · Gemini API |
| **[CSV-to-CRM Importer](https://github.com/karanthengane23/groweasy-csv-importer)** ([Live](https://groweasy-csv-importer-vert.vercel.app)) | Maps arbitrary lead-export columns into a fixed CRM schema using structured LLM extraction, with a 4-step wizard for large files | Next.js · TypeScript · Express · Gemini |

---

### 📊 GitHub Stats

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=karanthengane23&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true" alt="stats"/>
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=karanthengane23&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" alt="top languages"/>

<br/><br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=karanthengane23&theme=tokyonight&hide_border=true" alt="streak"/>

<br/><br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=karanthengane23&theme=tokyo-night&hide_border=true&area=true" alt="activity graph"/>

<br/><br/>

<img src="https://github-profile-trophy.vercel.app/?username=karanthengane23&theme=tokyonight&no-frame=true&no-bg=true&column=7&margin-w=8" alt="trophies"/>

<br/><br/>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/karanthengane23/karanthengane23/output/github-snake-dark.svg?v=1" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/karanthengane23/karanthengane23/output/github-snake.svg?v=1" />
  <img alt="github contribution snake" src="https://raw.githubusercontent.com/karanthengane23/karanthengane23/output/github-snake.svg?v=1" />
</picture>

</div>

---

<p align="center"><i>⚡ Fun fact: I'm a division-level chess player — same love for the endgame in code and on the board.</i></p>

<p align="center"><i>Open to full-time Software Engineer roles.</i></p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1b27,100:00C7B7&height=120&section=footer" width="100%" alt="footer"/>
